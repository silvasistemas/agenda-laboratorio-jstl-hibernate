...
public class DaoEntidade extends ImplementacaoCrud<Entidade> implements
	...
	public Date getUltimoAcessoEntidadeLogada(String login) {
		SqlRowSet rowSet = getJdbcTemplate().queryForRowSet(
				"select ent_ultimoacesso from entidade where ent_inativo is false and emaildb = ?",
				new Object[] { login });
	...
	public void updateUltimoAcessoUser(String login) {
		String sql = "update entidade  set ent_ultimoacesso = current_timestamp where ent_inativo is false and emaildb = ? ";
	...
	public boolean existeUsuario(String ent_login) {
		...
		builder.append(" select count(1) >= 1 from entidade where emaildb = '").append(ent_login).append("' ");
